...
!-----------MPI-------------
      include 'mpif.h'
      integer myid, num_of_procs, source 
      integer destination
      integer status(MPI_STATUS_SIZE)
      integer master, count, tag
      integer New_or_not
      integer current_id
      integer st
      integer ierror
...
! ------------------------------
      ...
      integer step
      double precision time_begin, time_end, MPI_WTIME(ierror)
! ++++++++++++++++++++++++++++++
...
! --------------------------
      ...
      print*, 'time = ', time_begin-time_end
...
!      print*, myid, 'received from ', master, ' NewOrNot=', New_or_not
      ...
      if (myid.eq.master) then !2.0
        do current_id= num_of_procs - 1, 0, -1 !do1
...
!     & Energy_current
          ...
          New_or_not = 1
          do st = 1, N_States
           if (dabs(Energy_current - Spectrum(st)).lt.1.d-5) then
            New_or_not = 0
            exit
           endif !Not new
          enddo !st
...
!-----------update spectr end       
           ...
           if (current_id.ne.master) then !2.2
               call MPI_SEND
     &          (New_or_not, count, 
...
! +++++ Write the configuration into files +++++++
     ...
     &         status = 'replace' )
        ...
        if (step.ge.10 .and. step.lt.100) then !!! STEP! 
         write(Nst2, '(i2)') step
         open( unit = 6,
     ...
     &         status = 'replace' )
        ...
        if (step.ge.100 .and. step.lt.1000) then!!! STEP! 
         write(Nst3, '(i3)') step
         open( unit = 6,
     ...
     &         status = 'replace' )
        ...
        if (step.ge.1000 .and. step.lt.10000) then!!! STEP! 
         write(Nst4, '(i4)') step !!! STEP! 
         open( unit = 6,
     ...
     &         status = 'replace' )
        ...
        if (step.ge.10000 .and. step.lt.100000) then!!! STEP! 
         write(Nst5, '(i5)') step !!! STEP! 
         open( unit = 6,
     ...
     &         status = 'replace' )
        ...
        if (step.ge.100000) stop 'Too many STEPS'!!! STEP! 
