...
string magnetic_preference="AFM";
double 			J0=0.2;
...
double  L, x, y, coordx[number_of_particles],
				 ...
				 dxs[number_of_particles],
				 dys[number_of_particles],
				 dzs[number_of_particles],
				 dxs_old[number_of_particles],
				 dys_old[number_of_particles],
				 dzs_old[number_of_particles];
...
double calc_energy(double coordx[], double coordy[], double dxs[], double dys[], double dzs[]);
//STRUCTURE

int main(){
	if(magnetic_preference=="FM"){
		J0=-J0;
	}
...
MSDwrite, VACFwrite, spinwrite, spinwrite2d;
write.open("data.txt");
...
//COORDINATES 0
		...
		for (int i=0; i<number_of_particles;  i++){
			...
			dys[i]=rand();
			dzs[i]=rand();
			double r=sqrt(dxs[i]*dxs[i]+dys[i]*dys[i]+dzs[i]*dzs[i]);
			dxs[i]=dxs[i]/r;
			dys[i]=dys[i]/r;
			dzs[i]=dzs[i]/r;
		}		
	...
	for (int i=0; i<number_of_particles_new;  i++){
		write <<  coordx[i] << " " << coordy[i] << " " <<"0" << " " << "0" << endl;
		spinwrite <<  coordx[i] << " " << coordy[i]<<  " " << 0 << " " << dxs[i] << " " << dys[i] <<  " " << dzs[i] << endl;
		spinwrite2d <<  coordx[i] << " " << coordy[i]<<  " " << dxs[i] << " " << dys[i] << endl;
	}
	
	write << "" << endl;
	write << "" << endl;
	E=calc_energy(coordx, coordy, dxs, dys, dzs);
	potwrite <<  0 << " " <<  E << endl;
//CYCLE
for (int N=1; N<NTIME; N++){
	...
	for (int i=0; i<number_of_particles_new; i++){
		...
		//theta[i]+=random(-d_phi*0.05*M_PI,d_phi*0.05*M_PI);
		dxs[i]=dxs[i]+random(-dr, dr);
		dys[i]=dys[i]+random(-dr, dr);
		dzs[i]=dzs[i]+random(-dr, dr);
		double r=sqrt(dxs[i]*dxs[i]+dys[i]*dys[i]+dzs[i]*dzs[i]);
		dxs[i]=dxs[i]/r;
		dys[i]=dys[i]/r;
		dzs[i]=dzs[i]/r;
		...
		E=calc_energy(coordx, coordy, dxs, dys, dzs);
		E_old=calc_energy(coordx_old, coordy_old, dxs_old, dys_old, dzs_old);
...
double calc_energy(double coordx[], double coordy[], double dxs[], double dys[], double dzs[]){
	...
	for (int i=0; i<number_of_particles_new; i++){
		for ( int j=0; j<number_of_particles_new; j++){
			...
			spin_scalar_product = (dxs[i]*dxs[j]+dys[i]*dys[j]+dzs[i]*dzs[j]);
			energy+=J0*pow((1-length[i][j]/r_c),3)*hevis*spin_scalar_product;
		}
